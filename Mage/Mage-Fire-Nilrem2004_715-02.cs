    ...
    public class FireMage : CombatRoutine
    {
		...
		public override void Pulse()
        {
			...
			if (openerwatch.ElapsedMilliseconds == 0)
                    {
						openerwatch.Start ();
						Log.WritePixelMagic("To activate/deactivate cooldowns press RIGHT CONTROL button", Color.Black);
						Log.WritePixelMagic("To activate/deactivate auto Combustion/Mirror Image press LEFT ALT button (cooldowns must be ENABLED)", Color.Black);
						Log.WritePixelMagic("it will use it when conditions are met and disable after that so you have to enable manually again", Color.Black);
						Log.WritePixelMagic("To activate/deactivate Opener press F1 (make sure it's not binded in WoW keybinds first!)", Color.Black);	
						Log.WritePixelMagic("To activate/deactivate using of Living Bomb press F2 (make sure it's not binded in WoW keybinds first!)", Color.Black);
						Log.WritePixelMagic("To activate/deactivate automatic interrupting F3 (it will interrupt above 80 percent of cast,)", Color.Black);
						Log.WritePixelMagic("To cast Meteor use MOUSE button 3, keep it pressed and cursor where you want to cast Meteor untill it's done.", Color.Black);
						Log.WritePixelMagic("To cast instant Flamestrike use Tilde button (left from number 1), keep mouse cursor where you want to cast Flamestrike.", Color.Black);
						Log.WritePixelMagic("OPENER is used on bosses, it requires Fire Blast(3 charges),Phoenix(3 charges),Mirror Image & Combustion off CD", Color.Black);
						Log.WritePixelMagic("When you enable OPENER, your task is to prepot and precast first spell, routine will do the rest including Mi/Combustion", Color.Black);
						Log.WritePixelMagic("You must first ENABLE COOLDOWNS to be able to use OPENER. When OPENER finishes it's work it will automatically DISABLE", Color.Black);
						Log.WritePixelMagic("itself so you must enable OPENER on next pull. This is to prevent nabness and unwanted routine behaviour.", Color.Black);
						Log.WritePixelMagic("For Flamestrike and Meteor you have to create macros /cast [@cursor] spell_name (Flamestrike or Meteor).", Color.Black);
                ...
                { 
					if (!Opener)
					{
						...
						{	
							Opener = !Opener;
							WoW.Speak("Opener "+ (Opener ? "Enabled" : "Disabled"));
							Log.WritePixelMagic("Opener "+ (Opener ? "Enabled" : "Disabled"), Color.Red);
							openerwatch.Restart();
						}
						else
						{
							WoW.Speak("Error");
							Log.WritePixelMagic("ERROR: You don't have all necessary CD's/CHARGES to enable OPENER or you didn't ENABLE COOLDOWNS first!", Color.Red);
							openerwatch.Restart();
						}
					}
					else
			...
			if (DetectKeyPress.GetKeyState(DetectKeyPress.VK_F2) < 0)
            {
                ...
                { 
					...
					Log.WritePixelMagic("Living Bomb "+ (UseLB ? "ON" : "OFF"), Color.Red);
                    LBwatch.Restart();
                }
            }
			...
			if (DetectKeyPress.GetKeyState(DetectKeyPress.Alt) < 0 && UseCooldowns)
            {
                ...
                { 
					...
					Log.WritePixelMagic("Automatic burst "+ (autoCD ? "ON" : "OFF"), Color.Red);
                    autoCDwatch.Restart();
                }
            }	
				...
				&& flamewatch.ElapsedMilliseconds > 1200)
			{
				...
				flamewatch.Reset ();
				Log.WritePixelMagic("Flamestrike CASTED", Color.Black);
				return;
			}
			...
			if (DetectKeyPress.GetKeyState(DetectKeyPress.VK_XBUTTON1) < 0 && WoW.IsInCombat && !WoW.IsSpellOnCooldown("Meteor") && WoW.CanCast("Meteor")
				&& meteorwatch.ElapsedMilliseconds > 1000)
				{
					...
					meteorwatch.Reset ();
					Log.WritePixelMagic("Meteor CASTED", Color.Black);
					return;				
				}
            ...
            if (combatRoutine.Type == RotationType.SingleTarget) 
            {
				...
				if (CombatWatch.IsRunning && CombatWatch.ElapsedMilliseconds < 4500)
					{
						...
						if (WoW.IsInCombat && !WoW.IsSpellOnCooldown("Combustion") && WoW.HasTarget && WoW.TargetIsEnemy && WoW.PlayerSpellCharges("Phoenix") == 2
							&& WoW.CanCast("Combustion") && WoW.CanCast("Mirror Image"))
							{
								...
								Log.WritePixelMagic("Opener Finished and disabled", Color.Red);
								CombatWatch.Reset();
								return;
							}
					...
					&& WoW.CanCast("Combustion") && WoW.CanCast("Mirror Image") && (WoW.PlayerHasBuff("HotStreak") || WoW.PlayerHasBuff("HeatingUp")))
				{
					...
					Log.WritePixelMagic("Burst done, disabling burst", Color.Red);
					return;
				}
