			...
			if (!WoW.PlayerHasBuff("Battle Cry")&&WoW.HasTarget&&WoW.IsInCombat&&WoW.TargetIsEnemy && WoW.IsSpellInRange("Mortal Strike"))
				{
					...
					if (WoW.TargetHealthPercent >= 20)
						//When targets are above 20%. Not in Execute phase.
						{
							...
							//FR Control
							if (WoW.CanCast("Focused Rage") &&WoW.Rage > 50&&WoW.SpellCooldownTimeRemaining("Battle Cry") > 6&&WoW.IsSpellOnCooldown("Mortal Strike")&&WoW.IsSpellOnCooldown("Colossus Smash"))
									{
									WoW.CastSpell("Focused Rage");
									}
							...
							//CS Control
							if (WoW.CanCast("Colossus Smash")&&!WoW.PlayerHasBuff("Shattered Defenses")&&WoW.SpellCooldownTimeRemaining("Battle Cry") >= 4&&!WoW.IsSpellOnCooldown("Colossus Smash"))
									{
									WoW.CastSpell("Colossus Smash");
									return;
									}
						...
						{  
							//Non BC ST
							if (WoW.CanCast("Colossus Smash")&&WoW.SpellCooldownTimeRemaining("Battle Cry") >= 4&&!WoW.IsSpellOnCooldown("Colossus Smash")&&!WoW.PlayerHasBuff("Shattered Defenses"))
									{
									WoW.CastSpell("Colossus Smash");
									return;
									}
