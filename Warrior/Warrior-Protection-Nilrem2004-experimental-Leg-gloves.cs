...
namespace PixelMagic.Rotation
{
    public class ArcaneMage : CombatRoutine
    {
        ...
        public override void Pulse()
        {
			...
			if (autointerrupt && WoW.IsInCombat && !WoW.IsSpellOnCooldown("Pummel") && WoW.HasTarget && WoW.TargetIsEnemy && WoW.IsSpellInRange("Shield Slam")
                ...
                if (WoW.HasTarget && WoW.TargetIsEnemy && !WoW.PlayerIsChanneling)
                {
...

						{
							if (WoW.CanCast("Shield Block") && WoW.Rage >= 15 && !WoW.PlayerIsChanneling &&
								(WoW.PlayerSpellCharges("Shield Block") == 2 || (WoW.PlayerSpellCharges("Shield Block") >= 1 && WoW.HealthPercent < 100 &&
									WoW.PlayerBuffTimeRemaining("ShieldBlockAura") <= 2) || WoW.PlayerSpellCharges("Shield Block") == 1 && WoW.HealthPercent < 100))
										{
							...
							if (WoW.IsSpellInRange("Shield Slam") && WoW.CanCast("Thunder Clap") && !WoW.IsSpellOnCooldown("Thunder Clap")
								&& (CombatWatch.ElapsedMilliseconds > 1000 && CombatWatch.ElapsedMilliseconds < 5000))
							{
								WoW.CastSpell("Thunder Clap");
								return;
							} 
						...
						/* ------------------ END IGNORE PAIN MANAGEMENT-------------------*/
							...
							if (WoW.CanCast("Revenge") && !WoW.IsSpellOnCooldown("Revenge") && WoW.IsSpellInRange("Shield Slam") && WoW.IsSpellOverlayed("Revenge") && !WoW.PlayerHasBuff("Vengeance: Ignore Pain") )
							{
							...
							if (WoW.CanCast("Revenge") && !WoW.IsSpellOnCooldown("Revenge") && WoW.IsSpellInRange("Shield Slam") && !WoW.PlayerHasBuff("Ignore Pain")
								&& WoW.Rage > 35 && WoW.HealthPercent < 100)
							{
								WoW.CastSpell("Revenge");
								return;
							}
							if (WoW.CanCast("Revenge") && !WoW.IsSpellOnCooldown("Revenge") && WoW.IsSpellInRange("Shield Slam") && (WoW.PlayerHasBuff("Ignore Pain") && WoW.PlayerBuffTimeRemaining("Ignore Pain") <= 3)
								&& WoW.Rage > 40 && WoW.HealthPercent < 100)
							{
								WoW.CastSpell("Revenge");
								return;
							}
							...
							if (WoW.CanCast("Victory Rush") && !WoW.IsSpellOnCooldown("Victory Rush") && WoW.IsSpellInRange("Shield Slam") && WoW.HealthPercent<90 && WoW.PlayerHasBuff("VictoryRush"))
							{
	...
	public class DetectKeyPress
    {
		...
		public static int VK_KEY_1 = 0x31;
